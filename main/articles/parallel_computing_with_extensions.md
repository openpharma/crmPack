...
`        ``for`` ``(``i`` ``in`` ``1``:``nObs``)`` ``{`\
`          ``y``[``i``]`` ``~`` ``dbern``(``mean``[``i``]``)`\
`          `[`logit`](https://docs.crmpack.org/reference/logit.md)`(``mean``[``i``]``)`` ``<-`` ``alpha0`` ``+`` ``alpha1`` ``*`` ``x``[``i``]`\
`        ``}`\
`      ``}``,`\
`      priormodel ``=`` ``function``(``)`` ``{`\
`        ``alpha0`` ``~`` `[`dnorm`](https://rdrr.io/r/stats/Normal.html)`(``mean1``, ``1`` ``/`` ``var1``)`\
`        ``alpha1`` ``~`` `[`dnorm`](https://rdrr.io/r/stats/Normal.html)`(``mean2``, ``1`` ``/`` ``var2``)`` ``%_%`` `[`I`](https://rdrr.io/r/base/AsIs.html)`(``0``, ``)`\
`      ``}``,`\
`      datanames ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"nObs"``, ``"y"``, ``"x"``)``,`\
`      modelspecs ``=`` ``function``(``)`` ``{`\
...
`      ``}``,`\
`      init ``=`` ``function``(``)`` ``{`\
`        `[`list`](https://rdrr.io/r/base/list.html)`(``alpha0 ``=`` ``mean1``, alpha1 ``=`` ``mean2``)`\
`      ``}``,`\
`      sample ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"alpha0"``, ``"alpha1"``)`\
`    ``)`\
`  ``}`\
\
`  ``## dose ----`\
...
`      ``checkmate``::`[`assert_subset`](https://mllg.github.io/checkmate/reference/checkSubset.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"alpha0"``, ``"alpha1"``)``, `[`names`](https://rdrr.io/r/base/names.html)`(``samples``)``)`\
`      `[`assert_length`](https://docs.crmpack.org/reference/check_length.md)`(``x``, len ``=`` `[`size`](https://docs.crmpack.org/reference/size.md)`(``samples``)``)`\
\
`      ``alpha0`` ``<-`` ``samples``@``data``$``alpha0`\
`      ``alpha1`` ``<-`` ``samples``@``data``$``alpha1`\
`      ``(`[`logit`](https://docs.crmpack.org/reference/logit.md)`(``x``)`` ``-`` ``alpha0``)`` ``/`` ``alpha1`\
`    ``}`\
`  ``)`\
\
`  ``## prob ----`\
...
`      ``)`\
`      ``checkmate``::`[`assert_subset`](https://mllg.github.io/checkmate/reference/checkSubset.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"alpha0"``, ``"alpha1"``)``, `[`names`](https://rdrr.io/r/base/names.html)`(``samples``)``)`\
`      `[`assert_length`](https://docs.crmpack.org/reference/check_length.md)`(``dose``, len ``=`` `[`size`](https://docs.crmpack.org/reference/size.md)`(``samples``)``)`\
\
`      ``alpha0`` ``<-`` ``samples``@``data``$``alpha0`\
`      ``alpha1`` ``<-`` ``samples``@``data``$``alpha1`\
`      ``1`` ``/`` ``(``1`` ``+`` `[`exp`](https://rdrr.io/r/base/Log.html)`(``-``alpha0`` ``-`` ``alpha1`` ``*`` ``dose``)``)`\
`    ``}`\
`  ``)`\
`}`
...
[`options`](https://rdrr.io/r/base/options.html)`(``crmpack_extensions ``=`` ``my_own_extension``)`
...
[`getOption`](https://rdrr.io/r/base/options.html)`(``"crmpack_extensions"``)``(``)`
...
`  doseGrid ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(`\
...
`my_data`` ``<-`` `[`Data`](https://docs.crmpack.org/reference/Data-class.md)`(`\
`  x ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``10``, ``20``, ``40``, ``80``, ``80``, ``160``, ``160``)``,`\
`  y ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``0``, ``0``, ``0``, ``0``, ``0``, ``1``, ``1``)``,`\
`  cohort ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``1``, ``2``, ``3``, ``4``, ``4``, ``5``, ``5``)``,`\
`  ID ``=`` ``1``:``7``,`\
...
`my_options`` ``<-`` `[`McmcOptions`](https://docs.crmpack.org/reference/McmcOptions-class.md)`(`\
`  burnin ``=`` ``100``,`\
`  step ``=`` ``2``,`\
`  samples ``=`` ``100``,`\
...
\
`# Create mcmc samples.`\
`my_samples`` ``<-`` `[`mcmc`](https://docs.crmpack.org/reference/mcmc.md)`(``my_data``, ``my_model``, ``my_options``)`\
\
`# Plot the dose toxicity curve.`\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``my_samples``, ``my_model``, ``my_data``)`\
\
`# Specify increments.`\
`my_increments`` ``<-`` `[`IncrementsRelativeDLT`](https://docs.crmpack.org/reference/IncrementsRelativeDLT-class.md)`(`\
`  intervals ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``0``, ``1``)``,`\
`  increments ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``1``, ``0.5``)`\
`)`\
\
`# Maximum dose.`\
`this_max_dose`` ``<-`` `[`maxDose`](https://docs.crmpack.org/reference/maxDose.md)`(``my_increments``, ``my_data``)`\
\
`# Next best dose.`\
`my_next_best`` ``<-`` `[`NextBestMinDist`](https://docs.crmpack.org/reference/NextBestMinDist-class.md)`(``target ``=`` ``0.3``)`\
`this_next_dose`` ``<-`` `[`nextBest`](https://docs.crmpack.org/reference/nextBest.md)`(`\
`  ``my_next_best``, ``this_max_dose``, ``my_samples``, ``my_model``, ``my_data`\
`)``$``value`\
\
`# Stopping rule.`\
`my_stopping`` ``<-`` `[`StoppingPatientsNearDose`](https://docs.crmpack.org/reference/StoppingPatientsNearDose-class.md)`(``nPatients ``=`` ``9``, percentage ``=`` ``0``)`\
\
`# Stop trial based on criteria and observed data.`\
[`stopTrial`](https://docs.crmpack.org/reference/stopTrial.md)`(``my_stopping``, ``this_next_dose``, ``my_samples``, ``my_model``, ``my_data``)`\
\
`# Cohorts size.`\
`my_size`` ``<-`` `[`CohortSizeDLT`](https://docs.crmpack.org/reference/CohortSizeDLT-class.md)`(`\
`  intervals ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``0``, ``1``)``,`\
`  cohort_size ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``1``, ``3``)`\
`)`\
\
`# Design.`\
`my_design`` ``<-`` `[`Design`](https://docs.crmpack.org/reference/Design-class.md)`(`\
`  model ``=`` ``my_model``,`\
...
\
`# Examine the design.`\
[`examine`](https://docs.crmpack.org/reference/examine.md)`(``my_design``, ``my_options``)`
...
`    ``my_model``,`\
`    alpha0 ``=`` `[`logit`](https://docs.crmpack.org/reference/logit.md)`(``intercept``)``,`\
`    alpha1 ``=`` ``(`[`logit`](https://docs.crmpack.org/reference/logit.md)`(``mtd_prob``)`` ``-`` `[`logit`](https://docs.crmpack.org/reference/logit.md)`(``intercept``)``)`` ``/`` ``mtd_dose`\
`  ``)`\
`}`\
\
...
`    ``dose``,`\
`    ``scenario`` ``=`` `[`cbind`](https://rdrr.io/r/base/cbind.html)`(``emptydata``@``doseGrid``, `[`c`](https://rdrr.io/r/base/c.html)`(`[`rep`](https://rdrr.io/r/base/rep.html)`(``0.05``, ``11``)``, `[`rep`](https://rdrr.io/r/base/rep.html)`(``0.80``, ``8``)``)``)``)`` ``{`\
`  ``scenario``[`[`match`](https://rdrr.io/r/base/match.html)`(``dose``, ``scenario``[``, ``1``]``)``, ``2``]`\
`}`\
\
`# Helper function that outputs the elapsed time.`\
`report_time`` ``<-`` ``function``(``report_text``)`` ``{`\
`  `[`cat`](https://rdrr.io/r/base/cat.html)`(`\
`    `[`format`](https://rdrr.io/r/base/format.html)`(`[`Sys.time`](https://rdrr.io/r/base/Sys.time.html)`(``)``, usetz ``=`` ``TRUE``)``,`\
`    ``report_text``,`\
`    ``"done - elapsed time from start:"``,`\
`    `[`round`](https://rdrr.io/r/base/Round.html)`(`[`difftime`](https://rdrr.io/r/base/difftime.html)`(`[`Sys.time`](https://rdrr.io/r/base/Sys.time.html)`(``)``, ``start_time``, units ``=`` ``"mins"``)``, digits ``=`` ``1``)``,`\
`    ``"\n"`\
`  ``)`\
`}`\
\
...
`    nCores ``=`` ``parallelly``::`[`availableCores`](https://parallelly.futureverse.org/reference/availableCores.html)`(``)`\
`  ``)`\
`}`\
\
`# get operation characteristics without utilizing parallel computing for`\
`# selected truth (to reduce the run time).`\
`time_no_parallel`` ``<-`` `[`system.time`](https://rdrr.io/r/base/system.time.html)`(``{`\
`  ``start_time`` ``<-`` `[`Sys.time`](https://rdrr.io/r/base/Sys.time.html)`(``)`\
`  `[`cat`](https://rdrr.io/r/base/cat.html)`(`[`format`](https://rdrr.io/r/base/format.html)`(`[`Sys.time`](https://rdrr.io/r/base/Sys.time.html)`(``)``, usetz ``=`` ``TRUE``)``, ``"start"``, ``"\n"``)`\
\
`  ``my_nsim`` ``<-`` ``10`\
`  ``do_parallel`` ``<-`` ``FALSE`\
\
...
`time`` ``<-`` `[`system.time`](https://rdrr.io/r/base/system.time.html)`(``{`\
`  ``start_time`` ``<-`` `[`Sys.time`](https://rdrr.io/r/base/Sys.time.html)`(``)`\
`  `[`cat`](https://rdrr.io/r/base/cat.html)`(`[`format`](https://rdrr.io/r/base/format.html)`(`[`Sys.time`](https://rdrr.io/r/base/Sys.time.html)`(``)``, usetz ``=`` ``TRUE``)``, ``"start"``, ``"\n"``)`\
\
`  ``my_nsim`` ``<-`` ``10`\
...
`  `[`dump`](https://rdrr.io/r/base/dump.html)`(``"my_own_extension"``, file ``=`` ``"user_extension.R"``)`\
`  ``file_con`` ``<-`` `[`file`](https://rdrr.io/r/base/connections.html)`(``"user_extension.R"``)`\
`  ``tmp`` ``<-`` `[`readLines`](https://rdrr.io/r/base/readLines.html)`(``file_con``)``[``-`[`c`](https://rdrr.io/r/base/c.html)`(``1``:``3``, ``135``)``]`\
`  ``tmp`` ``<-`` `[`gsub`](https://rdrr.io/r/base/grep.html)`(``"<<-"``, ``"<-"``, ``tmp``)`\
`  `[`writeLines`](https://rdrr.io/r/base/writeLines.html)`(``tmp``, ``file_con``)`\
\
`  ``# Source the stored file in the wrapper function.`\
`  ``my_own_extension2`` ``<-`` ``function``(``)`` ``{`\
`    `[`source`](https://rdrr.io/r/base/source.html)`(``"user_extension.R"``)`\
`  ``}`\
\
`  `[`options`](https://rdrr.io/r/base/options.html)`(``crmpack_extensions ``=`` ``my_own_extension2``)`\
`  `[`getOption`](https://rdrr.io/r/base/options.html)`(``"crmpack_extensions"``)``(``)`\
\
