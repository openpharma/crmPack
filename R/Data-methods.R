...
#'
#' @note The main motivation behind this function is related to the `openBUGS`.
#'   If there is only one observation, the data is not passed correctly to
#'   `openBUGS`, i.e. e.g. `x` and `y` are treated like scalars in the data
#'   file. Therefore it is necessary to add dummy values to the vectors in this
#'   case. As we don't change the number of observations (`nObs`), this addition
#'   of zeros doesn't affect the results of `openBUGS` computations.
#'
...
setGeneric(
  name = "add_dummy",
  def = function(object, where, dummy = 0, ...) {
    standardGeneric("add_dummy")
  },
  valueClass = "Data"
)
...
#' @rdname add_dummy
#' @aliases add_dummy-Data
#' @example examples/Data-method-add_dummy.R
#'
setMethod(
  f = "add_dummy",
  signature = signature(object = "Data"),
  definition = function(object, where, dummy = 0, ...) {
    assert_number(dummy)
