...
#'
#' @export
h_convert_ordinal_samples <- function(x, grade) {
  # Validate
  assert_integer(grade, len = 1, lower = 1)
  assert_class(x, "Samples")
  assert_subset(c(paste0("alpha[", 1:grade, "]"), "beta"), names(x@data))
  # Execute
  d <- list("alpha0" = x@data[[paste0("alpha[", grade, "]")]], "alpha1" = x@data$beta)
  Samples(data = d, options = x@options)
}
