...
##' --------------------------------------------------
##' NCRM method with loss function & overdose control
##' --------------------------------------------------
          ...
          def =
            function(nextBest, doselimit, samples, model, data, ...) {
              ...
              if (length(nextBest@overdose) > 2) {
                ...
                probUnacceptable <-
                  colMeans((probSamples > nextBest@overdose[2]) &
                             (probSamples <= nextBest@overdose[3]))
              ...
              if (length(nextBest@overdose) > 2) {
                ...
                plotJoint <- ggplot(data = dat, aes(x, y.values, fill = y.ind)) +
                  ...
                  scale_fill_manual("",
                                    labels = c("Pr(Unacceptable)", "Pr(Excessive)", "Pr(Target)", "Pr(Underdosing)"), values = c("underdosing" = "steelblue3", "target" = "mediumseagreen", "excessive" = "orange", "unacceptable" = "red1"),
              ...
              if (length(nextBest@overdose) <= 2) {
                ...
                plotJoint <- ggplot(data = dat, aes(x, y.values, fill = y.ind)) +
                  ...
                  theme(legend.position = "bottom", legend.title = element_blank()) +
                  scale_fill_manual("",
                                    labels = c("Pr(Overdose)", "Pr(Target)", "Pr(Underdosing)"), values = c("underdosing" = "steelblue3", "target" = "mediumseagreen", "overdose" = "red1"),
                                    guide = guide_legend(reverse = TRUE)
                  )
              }
