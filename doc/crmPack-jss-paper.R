...
knitr::opts_chunk$set(abbreviate = 0, wrap = FALSE)
Sys.setenv(
  TEXINPUTS = getwd(),
  BIBINPUTS = getwd(),
  BSTINPUTS = getwd()
...
oh <- knitr::knit_hooks$get("output")
knitr::knit_hooks$set(output = function(x, options) {
  ...
  if (options$abbreviate > 0) {
    ...
    ret <- if (length(ret) > options$abbreviate * 2) {
      paste(
        paste(head(ret, options$abbreviate),
          collapse = "\n"
        ),
        "\n...\n",
...
design <- Design(
  ...
  cohortSize = mySize, pl_cohort_size = mySizePL,
...
## ----OneParExp-model-example, fig.width=5,fig.height=4,out.width='.5\\linewidth',fig.align='center', fig.env='figure', fig.cap='Model fit of the one parameter power model'----
...
(newNextDoseVal <- nextBest(
  newMyNextBest, nextMaxDose,
  newSamples, newModel, data
)$value)
