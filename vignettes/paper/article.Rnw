...
\end{equation}
for $1=1,...,s$ with $ s \leq n$ and $d_i$ represents the dose level given to subject $i$. $\epsilon_i$ the random error term $\epsilon_i \sim N(0, \sigma^2)$ of subject $i$. 
%The reason of using a linear log-log model for dose-efficacy relationship is also described in \cite{yeung2015}
...
The efficacy model will be specified with the \texttt{Effloglog} class as
<<Effmodel>>=
...
A study will be stopped when all dose levels under consideration are unsafe. Therefore, we will not allow subjects to administer to dose levels if the risk of DLTs is unacceptable. The safety constraints to be used are described in \cite{yeung2015}.
Two DLTS limits for DLTs risk (probability of the occurrence of DLT) have to be fixed.: 1) $\gamma^{**}$ is the highest risk to be faced by subjects during the trial and 2) $\gamma^{*} \leq \gamma^{**}$ is the highest risk for subsequently treated subjects for future studies. The corresponding limiting doses are $d^*= {\rm TD}100\gamma^*$ and $d^{**}={\rm TD}100\gamma^{**}$, respectively. The slightly higher risk of DLE allowed during the study will allow better exploration of the region of interest and hence improved
...
In this package, all escalation rules are contained in the \texttt{NextBest} class. There are two main types of rules: either only the binary DLTs responses are incorporated, the \texttt{NextBestTD}, \texttt{NextBestTDSamples} classes; or both the binary DLTs and the continuous efficacy response are incorporated, the \texttt{NextBestMaxGain} and \texttt{NextBestMaxGainSamples} classes can be used with dose-responses models defined under \texttt{ModelPseudo} class. We set $\gamma^*=0.3$ and $\gamma^{**}=0.35$ as the two safety limits.

The escalation rules \texttt{NextBestTDSamples} and \texttt{NextBestMaxGainSamples} can be used when DLT samples or both DLT and efficacy samples are generated and involved in the escalation procedures.

Back to our example assuming no samples are generated for DLT and efficacy response and the gain fucntion is used for the escalation rule. We have
<<NextBestGain>>=
GainNextBest <-NextBestMaxGain(DLEDuringTrialtarget=0.35,
                               DLEEndOfTrialtarget=0.3)
@

Both the target probability of the occurrence of a DLTs to be used during, \code{DLEDuringTrialtarget} slot and at the end of the trial, slot \code{DLEEndOfTrialtarget} need to be specified. 
%When DLTs and efficacy samples are involved, two further functions have to be specified in the corresponding class. Based on posterior samples generated, one function, slot \code{TDderive}, will be use to derive the estimates of dose corresponds to the target probability of DLTs to be used during and at the end of the trial, and the other, slot \code{Gstarderive}, will be used to derive the estimates of Gstar, the dose which gives the maximum gain value. In here, we will recommend 30\% posterior quantile of the samples to be estimates for the doses corresponding to the \code{DLEtargetDuringTrialtarget} and \code{DLEEndOfTrialtarget} doses and 50\% posterior quantile of the samples to be estimates for the doses corresponding to the Gstar, the dose which gives the maximum gain value.
...
Assuming we have some observations capture in data set \texttt{data3}
...
From the example above, first we updates the model (their model parameters) with the \texttt{update} function. Then using \texttt{nextBest} the escalation rule (Class) is \texttt{GainNextBest}, the maximum allowable dose is specified in slot \code{doselimit}, the DLT model in slot \code{model}, the efficacy model in slot \code{Effmodel} and the data set in slot \code{data}. In addition, we can use the \verb|$| operator to look at specific content from the results. For example,
<<doseRecommendation>>=?
...
As described in earlier section, a design has to be built before running the simulations. In this example, we will specify our increment restriction as
<<Increments>>=
myIncrements <- IncrementsRelative(intervals=c(25,300),
                                 increments=c(2,2))
@
which means a maximum of 3-fold increase for dose levels between 25 to 300 mg are defined.

Then the design for the simulations can be defined. The class objects \texttt{TDDesign}, \texttt{TDsamplesDesign}, \texttt{DualResponsesDesign} and \texttt{DualResponsesSamplesDesign} can be used with models defined under \texttt{ModelPseudo} class. In here, We use the \texttt{DualResponsesDesign} class object. We want to start the simulations with an empty data set and start with the lowest dose 25 mg.
...
It is similar to how we specify the design for the CRM trial. The only difference is the right design class (\texttt{DualResponsesDesign}), the right escalation (\texttt{GainNextBest}) and an additional slot for the efficacy model have to be used.  

Then we have to specify the scenario for the simulation, that is the true DLT and efficacy curves that we will using.
...
From the example, we specify the true value of the precision (\code{trueNu}) of the efficacy response as 1/0.025 which is a true variance of value 0.025. The slot \code{arg} could contain additional arguments for the \code{trueDLE} or \code{trueEff} function and default as \code{NULL}. The number of simulations are specified in \code{nsim} and the random number generator seed in \code{seed}. The slot \code{parallel} can be used to enable running the simulations in parallel with all processors in the computer. 
The results \code{Sim1} will be put into one of the sub-class defined under the parent class \texttt{GeneralSimulations}, the \texttt{PseudoDualSimulations} class object. 

%There are also multiple slots under this class. Descriptions of slots contains in the simulated results can be found at help pages of the corresponding class objects. For example, the \code{data} slot contain a list of data produced of the simulated trial and we can plot the course of the fourth simulated trial and find the final recommended dose of this trial by
%<<plot-sim>>=
%print(plot(Sim1@data[[4]]))
%Sim1@doses[4]
%@
Similarly, we can also plot, summarized and plot the summary of the simulated results 
%such as
%<<resultsplotsummary>>=
%SUM1 <- summary(Sim1,
%        trueDLE=myTruthDLT,
...
As described, the \pkg{crmPack} is constructed using S4 classes and methods. This allows users to extend our package easily by constructing their own codes via setting up new S4 classes and corresponding methods that can be used in junction with other existing classes and methods defined in the package.
