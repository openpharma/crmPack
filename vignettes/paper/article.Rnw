...
\subsection{Implementing a CRM trial}\label{crmtrial}
...
There are two sub-classes under \texttt{ModelPseudo} class are the \texttt{ModelTox} class which contains all DLTs models and the \texttt{ModelEff} class which includes all efficacy models. The two-parameter dose-DLTs model described is defined as \texttt{LogisticIndepBeta} class model in the package and contained in \texttt{ModelTox} class. The linear log-log  described are defined as \texttt{Effloglog} class model contained in \texttt{ModelEff} class. Now we will demonstrate how we define these models based on the same example used in Section \cite{crmtrial} and \cite{yeung2015}. The binary DLTs endpoint considered was described in Section \cite{crmtrial} and the continuous efficacy endpoint considered is a transformed form of the DPP4 activity described in \cite{yeung2015}. 
