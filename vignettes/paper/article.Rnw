...
\end{equation}
such that $w(x) \sim N(\E(w(x)), \sigma^2)$. 
$c$ is an integer constant. The default value is $c=0$ and it can be chosen for $c \geq 1$ when any of the doses considered are less than or equal to 1. Throughtout the examples in this paper, we will use $c=1$.
