...
In order to protect the probands undergoing the first tests of new investigational agents in humans from too toxic doses, Phase I trials start with low doses and then escalate to higher doses in a sequential fashion. The probands can either be healthy volunteers (e.g.\ in neurology) or patients (e.g.\ in oncology), and we will henceforth use only the latter for ease of presentation. While higher doses of agents are usually expected to deliver stronger pharmacodynamic effects and hence improved efficacy, higher doses also usually cause more and more severe adverse events in the patients. In order to simplify the decision making, before starting the trial usually binary dose-limiting toxicities (DLTs) are defined (e.g.\ adverse events reaching specific severity levels). Historically, patients were treated in cohorts of three, with the dose for the next cohort then being determined algorithmically from the number of DLTs having been observed in the previous cohort.

Algorithmic designs like the simple 3+3 design \citep{carter1973} have disadvantages that have been recognized in the statistics community, see e.g.\ \cite{Paoletti2015}. Fundamentally the escalation rules of the 3+3 design do not have a statistical justification \citep{Storer1989}. Moreover, they cannot be extended to the challenges of today's Phase I trials, with extension cohorts, dose escalation of drug combinations and optimal biological dose determination, naming just a few prominent examples. Hence model-based dose escalation designs like the continual reassessment method \citep[CRM,][]{oquigley1990} have gained increasing interest due to the need for more efficient and informative Phase I trials. These designs are based in statistical inference, with dose-toxicity regression models as the backbone, and are therefore flexible for adaptation to various complex trial designs. Importantly, they avoid fixing only a few dose levels in advance.

However, the wide-spread implementation of such designs has been hindered by the need for either licensing specialized commercial software (thus losing flexibility) or programming the design and simulations from scratch for each project (thus losing efficiency). While the models underlying most model-based dose-escalation procedures can easily be fit in standard software with the capability to fit generalized linear models (e.g. \code{PROC MIXED} in \proglang{SAS}, \code{glm} in \proglang{Stata} or \proglang{R}), there are still only few software solutions available dedicated to dose-escalation studies.

The commercial packages \proglang{East} (Cytel, Cambridge, MA) and \proglang{ADDPLAN} (ICON, Dublin, Ireland) both offer extensions to their basic design software for dose-escalation studies (\pkg{ESCALATE} in \proglang{East} and \pkg{df} in \proglang{Addplan}) implementing the algorithmic 3+3 design and various versions of the CRM. Similarly \proglang{FACTS} \citep{FACTS} also offers different common dose-escalation methods. Due to the commercial nature of these implementations there is, however, a limitation on how much the designs can be tailored towards the individual needs of the study. Similarly static implementations of methods for dose-escalation are available in the \proglang{Stata} module \pkg{crm} \citep{StataCRM} which implements the CRM and the  \pkg{dfcrm} package in \proglang{R} which additionally implements the time-to-event CRM (TITE-CRM) \citep{cheung2000}. Finally the \pkg{bcrm} package \citep{sweeting2013} offers a wide range of different dose-escalation designs. It implements a variety of one and two parameter models, facilitates different ways to specify prior distributions, escalation and stopping rules.\\
...
In Section~\ref{sec:framework} we describe the underlying structure of the package while Section~\ref{sec:using} provides some example of the usage of the package including how to add a custom method before we conclude with a discussion (Section~\ref{sec:discussion}). \\
...
in \cite{Thall2010}. Before we start, we have to install and subsequently load our package in R:
<<start, echo=FALSE>>=
library("crmPack")
@
...
is observed, typically a safety measurement as e.g.\ the binary DLT $y \in \{0,
1\}$. Grouping together $n_{j}$ patients in cohort $j$, generating the cohort
$j$ data ${\cal C}_{j} = \{(x_{j,1}, y_{j,1}), \dotsc, (x_{j,n_{j}}, y_{j,n_{j}})\}$, we
can denote the data generated from the first $N$ cohorts as ${\cal D}_{N}= {\cal
  C}_{1} \cup \dotsb \cup {\cal C}_{N}$.

In the simple case of a univariate dose $x$ resulting in binary DLT observations
$y$, the S4 class \code{Data} provided by \pkg{crmPack} can be used. Objects of this
class can be created by calling the accompanying initialization function of the
...
@
%
The argument \code{x} takes the doses $x_{1,1}=0.1, x_{2,1} = 0.5, \dotsc,
x_{6,3} = 10$ where \code{doseGrid} captures the set ${\cal X}$ of all possible
doses, \code{y} takes the binary DLTs (here $y_{6, 2} = 1$ denotes the only DLT
having been observed in the 2nd patient in the 6th cohort), while \code{cohort}
groups the patients together in cohorts (here in total $N=6$).

\paragraph{Model}
\label{sec:frame:model}
%
The core of model-based dose escalation designs is the model. Taking the more
general Bayesian inference approach, the model in \pkg{crmPack} consists of
firstly a probability density function $f(y \given x, \theta)$ or probability
mass function $\Prob(Y=y \given x, \theta)$ of $y$ for a patient who receives
treatment $x$ assuming the parameter (vector) $\theta$ (this is the likelihood
of $\theta$), and secondly a prior $p(\theta \given \xi)$ for $\theta$ given
fixed hyperparameters $\xi$.

One popular example \citep{Neuenschwander2008} is the logistic regression model
with probability mass function
\begin{equation}
\label{eq:LogisticLogNormal}
\mathrm{logit}(\Prob(Y=1 | x,\theta))
\equiv logit(\pi(x,\theta))
= \alpha + \beta log\left(\frac{x}{x^*}\right)
\end{equation}
with parameter vector $\theta = (\alpha,\beta)$, dose $x > 0$ and specified
reference dose $x^{*}$. The prior is typically specified via a bivariate normal
distribution
\begin{equation}
...
$\mu$ and the prior covariance matrix $\Sigma$. This then induces the prior
density function $p(\theta \given \xi)$ on the original parameter vector
$\theta$.
%
In \pkg{crmPack} the class \code{LogisticLogNormal} can be used, for example:
<<Model-example>>=
model <- LogisticLogNormal(mean = c(-0.85, 1),
                           cov = matrix(c(1, -0.5, -0.5, 1), nrow = 2),
                           refDose = 50)
@
Note that, as always, the initialization function is used to create the object.
Here \code{refDose} takes the reference dose $x^{*} = 50$, \code{mean} the prior
mean vector $\mu = (-0.85, 1)$ and \code{cov} the prior covariance matrix
$\Sigma=
...
\subsection{Implementing a CRM trial}\label{subsec:crmtrial}

We consider a trial in Type II diabetes carried out by Hoffmann-La Roche Ltd. For each subject, one response each of DLT and efficacy was observed. The DLT response is equal to 1 if any abnormal symptom is observed and 0 otherwise. In this Section we will show how to implement a CRM design for dose-escalation based on safety only.

Suppose that 12 dose levels ranging from 25 to 300 mg in 25 mg increments of a
novel agent are available, defining our dose-grid, with 25 mg being our starting
dose. Our primary objective in conducting the dose-escalation study is to
characterize the dose-toxicity relationship and to estimate the MTD. We choose
to use in this case a Bayesian Logistic regression with overdose control
\citep{Neuenschwander2008} as described in \eqref{eq:LogisticLogNormal}.
...
The procedures described below are proposed in \cite{yeung2015}. Two models were used to describe the relationship between the responses and their corresponding dose levels. Assume that a total of $k$ dose levels are considered and they are of ascending order such that $d_{(1)} < ... < d_{(k)}$.\\
...
for $j=1,...,k$. The parameters $\phi_1$ and $\phi_2 > 0 $ are the corresponding intercept and slope parameter of this model.

Before assign the first dose to the first or first cohort of subjects, we will express our initial beliefs concerning all unknown model parameters in the form of prior distribution. In here, the prior will be expressed in form of pseudo data, some imaginary data as if for some observations we already had before the trial starts. Details about how to set up prior inform of the pseudo data is described in \cite{yeung2015}
...
%We express this opinion with some hypothetical number of number of subjects. Let $l=-1,0$ for $n_l$ subject treated at dose $l$, there will be $t_l$ subjects observed with DLTs and $u_l$ non-DLTs such that $p_l=t_l/n_l$. 
As stated in \cite{w06}, based on the pseudo-data, the prior modal estimates of $\phi_1$ and $\phi_2$ obtained are the same as the maximum likelihood estimates which can be computed using standard statistical software fits. When more DLTs observations are available, standard software fits can also be used to obtain posterior modal estimates for the model parameters.\\

\textit{Efficacy models}\\

For the continuous efficacy endpoint, the linear log-log model can be used to describe the dose-efficacy relationship. In here, efficacy responses of subjects without DLTs will be considered. Let $n$ be the total number of subjects treated in a study. Given no DLTs responses, the efficacy response of subject $i$ is represented by
\begin{equation}\label{loglog}
w_i = \theta_1 + \theta_2 {\rm log(log}d_i) +\epsilon _i
\end{equation}
for $1=1,...,s$ with $ s \leq n$ and $d_i$ represents the dose level given to subject $i$. $\epsilon_i$ the random error term $\epsilon_i \sim N(0, \sigma^2)$ of subject $i$. The reason of using a linear log-log model for dose-efficacy relationship is also described in \cite{yeung2015}

%It is always common to use the ${\rm E_{max}}$ model to estimate the dose-efficacy relationship in clinical pharmacology context. In particular, we are interested to estimate efficacy curve under the ${\rm E_{max}}$ model with a shape such that the efficacy responses will increase sharply with increasing dose at lower dose levels and form a plateau at higher dose level. The advantages of using a linear log-log model instead are firstly this model give a similar shape than to the ${\rm E_{max}}$ model and secondly we only need to fit the intercept, $\theta_1$ and the slope, $\theta_2$ parameter instead of fitting four parameters in the ${\rm E_{max}}$ model.

Similarly, initial beliefs about the parameters $\theta_1$ and $\theta_2$ in this model are expressed in terms of pseudo-data as described in \cite{yeung2015}. Using the pseudo data and if any all available efficacy responses,  
 %First two dose levels $d_{-1}$, usually the lowest and $d_{0}$, the highest dose levels are fixed and elicited clinical pharmacologists' opinion about the efficacy responses $w_{-1}$ and $w_0$ corresponding to the two fixed log-log dose levels $l_{-1}$ and $l_0$. 
 the prior or the posterior modal estimates of $\theta_1$ and $\theta_2$ can be obtained using standard fits in statistical software. 
 %Furthermore, given more observed responses of efficacy, the posterior modal estimates of the model parameters can be obtained using standard software fits.

The precision $\nu=\sigma^{-2}$, can be specified with a fixed value or gamma distributions will be used as the prior and posterior distributions.



\textbf{Corresponding Model classes in crmPack package}

The \texttt{ModelPseudo} class contains all model classes where their prior are specified in terms of pseudo data and when standard maximum likelihood procedures are used to obtain prior and posterior estimates. All models included in this package will contains in either the \texttt{GeneralModel} or \texttt{ModelPseudo} class. In particular, when the two endpoints are modeled separately, all corresponding dose-responses models are contained in \texttt{ModelPseudo} class.

There are two sub-classes under \texttt{ModelPseudo} class are the \texttt{ModelTox} class which contains all DLTs models and the \texttt{ModelEff} class which includes all efficacy models. The two-parameter dose-DLTs model described is defined as \texttt{LogisticIndepBeta} class model in the package and contained in \texttt{ModelTox} class. The linear log-log  described are defined as \texttt{Effloglog} class model contained in \texttt{ModelEff} class. Now we will demonstrate how we define these models based on the same example used in Sub-Section \ref{subsec:crmtrial} and \cite{yeung2015}. The binary DLTs endpoint considered was described in Sub-Section \ref{subsec:crmtrial} and the continuous efficacy endpoint considered is a transformed form of the DPP4 activity described in \cite{yeung2015}. 
...
We want to find the optimal dose level which represents the best trade-off between the DLTs and efficacy responses. 
We can set up our data set before the trial start when no observations are available. That is we only need to specify the dose level that will be used for the trial
<<Data>>=
data1 <- Data(doseGrid=seq(25,300,25))
data2 <- DataDual(doseGrid=seq(25,300,25))
@

Both \code{data1} and \code{data2} are empty data set. If only DLTs response is incorporated into the escalation process, empty data set \code{data1} of class \texttt{Data} can be used. If both DLTs and efficacy responses are incorporated into the procedure, empty data set \code{data2} of class \texttt{DataDual} can be used.


Then we have to know the dose to be given to the first cohort of subjects. The initial beliefs of unknown parameters are expressed in form of pseudo-data. A conservative prior will be used representing what we fear rather than what we believe. 

The pseudo-data for the prior of the DLTS are described as follows.
Assuming 3 subjects each are treated at the lowest (25mg) and the highest (300mg) dose level and clinical pharmacologists' opinions for the number of DLTs observed are 1.05 and 1.8 at these two dose levels. That is the probability of a DLTs occurring at 25 and 300 mg are 0.35 and 0.6, respectively.  
...
The prior of the model parameters are expressed in terms of pseudo data for \texttt{LogisticIndepBeta} class model. The slot \code{DLEdose} represented the two dose levels that we fixed, 25 and 300mg. The vector $c(3,3)$ specified in the slot \code{DLEweights} means that assumed 3 subjects are treated at each of the two dose levels. The slot \code{binDLE} represented the number of DLTs 1.05 and 1.8 observed at the two corresponding dose levels out of the 3 subjects treated at each of the two dose levels. Since no real observations are available before the trial start, we will specify the \code{data} slot of this model with the empty data set \code{data1} of class \texttt{Data}. Note that, since at least two pseudo responses from two different dose levels are need to obtain the prior modal estimate of the intercept and the slope of the model, slots \code{binDLE}, \code{DLEweights} and \code{DLEdose} must be specified in form of a vector of at least length 2 and the order of their elements must be consistent to the other two vectors.


Similarly, for the efficacy linear log-log model, the prior are expressed in terms of pseudo data. Clinical pharmacologists' opinions were obtained for efficacy responses at the lowest and the highest dose level and interpreted as 1.223 and 2.513 corresponds to the two dose levels if one subjects are treated at each of these doses. We will use a gamma distribution as the prior of the precision $\nu=\sigma^{-2}$ with $a=1$, the shape parameter and $b=0.025$, the rate parameter and $\sigma^2=\{E(\nu)\}^{-1}=0.025$ as the prior modal value.
...
The slot \code{Eff} represents the vector of pseudo efficacy responses at the two fixed dose levels. Each of these efficacy responses is given by assuming one subject are treated at each of these dose levels. Slot \code{Effdose} is the vector for the fixed dose levels. We can specified here with a fixed value or with values of the parameters of the prior gamma distribution of the precision $\nu$. That is if a prior gamma distribution, the shape $a$ and the rate parameter $b$ have to be specified in the slot \code{nu}. A data set of \texttt{DataDual} class like the emptydata set \code{data2} has to be used for slot \code{data}. 

Furthermore, the structure of these model can be viewed using \code{str} function, to access to their slots using the \verb|@| operator and \code{update} to update the current model to obtain new posterior model estimates. For example,  
<<stratupdate>>=
##structure of the DLT model
str(DLTmodel)
##Look at slot phi1/ obtain prior modal estimates of phi1 in DLT model
DLTmodel@phi1
...
Furthermore, instead of using the prior or posterior estimates for the model parameters for evaluation, samples of model parameters for models defined under \texttt{ModelPseudo} class can also be generated. The \code{mcmc} function described earlier can also be used for any models defined under \texttt{ModelPseudo} class .
...
%The MCMC sampling are controlled with an object of class \texttt{McmcOptions} and now the object \code{options} specifies that 2000 parameter samples will be created with a "burn-in" phase of 100 iterations that are discarded and save a sample every 2 iterations. We can also use \code{str()} to look at the structure of the samples and \code{get} function to extract samples of the model parameters such as
...
A study will be stopped when all dose levels under consideration are unsafe. Therefore, we will not allow subjects to administer to dose levels if the risk of DLTs us unacceptable. The safety constraints to be used are described in \cite{yeung2015}.
Two DLTS limits for DLTs risk (probability of the occurrence of DLT) are fixed.: 1) $\gamma^{**}$ is the highest risk to be faced by subjects during the trial and 2) $\gamma^{*} \leq \gamma^{**}$ is the highest risk for subsequently treated subjects for future studies. The corresponding limiting doses are $d^*= {\rm TD}100\gamma^*$ and $d^{**}={\rm TD}100\gamma^{**}$, respectively. The slightly higher risk of DLE allowed during the study will allow
...
If only the DLTs responses are incorporated into the dose-escalation process, the maximum dose level closest to the posterior estimate of $d^{**}$ will be given to the next or next cohort of subject. At the end of a trial, the maximum dose level closest to the final posterior estimate of $d^*$ will be recommended. (SO approach \cite{yeung2015})


\textit{Escalation rule}\\

A gain function to reflect the trade-off between efficacy and safety is used to determine the dose for the next or next cohort of subjects. Details of the gain function is also described in in \cite{yeung2015} and in particular one specific form of the gain function will be used in this example. To represent no gain when a DLT is observed on a subject, the gain function  associated with dose $j$ is defined as 
\begin{equation}
G_{(j)}=\frac{E(w_{(j)})}{1+{\rm exp}(\phi_1+\phi_2 {\rm log} d_{(j)})}
\end{equation}
for $j=1,...,k$ where $w_{j}$ are the efficacy responses observed at dose $j$ given that no DLT is observed. 

This gain function is represented as the expected efficacy responses times the probability of the occurrence of a DLTs. Both the components for the efficacy and DLTs responses are presented in one equation to reflect the trade-off between efficacy and safety. The maximum dose level which gives the maximum gain value and satisfies the safety constraints will be administer to next or next cohort of subjects. We also let $g^*$ be the dose which gives the maximum gain value. 
In other words, the minimum of the posterior estimate of $d^{**}$ and $g^*$ will be selected. The maximum dose level closest to the selected quantity will be the next dose to be administered. At the end of the trial, the maximum dose level closest to, the minimum of the posterior estimate of the $d^*$ and $g^*$, will be recommended for subsequent trials. (SE approach \cite{yeung2015})\\

\textbf{Corresponding NextBest classes in crmPack package}\\

In this package, all escalation rules are contained in the \texttt{NextBest} class. There are two main types of rules: either only the binary DLTs responses are incorporated or both the binary DLTs and the continuous efficacy response are incorporated. In particular, classes \texttt{NextBestTD}, \texttt{NextBestTDSamples}, \texttt{NextBestMaxGain} and \texttt{NextBestMaxGainSamples} can be used with dose-responses models defined under \texttt{ModelPseudo} class. The first two classes relates to escalation rule based only on the DLTs response using the safety constraints only as escalation rules where \texttt{NextBestTDSamples}is a rule when DLT samples are involved in the escalation procedure. We set $\gamma^*=0.3$ and $\gamma^{**}=0.35$ as the two safety limits.

When DLTs and efficacy responses are both incorporated into the procedure, the escalation rules \texttt{NextBestMaxGain} and \texttt{NextBestMaxGainSamples} can be used and the later rule refers to the procedure when both DLT and efficacy samples are involved. 
...
Both the target probability of the occurrence of a DLTs to be used during, \code{DLEDuringTrialtarget} slot and at the end of the trial, slot \code{DLEEndOfTrialtarget} need to be specified. When DLTs and efficacy samples are involved, two further functions have to be specified in the corresponding class. Based on posterior samples generated, one function, slot \code{TDderive}, will be use to derive the estimates of dose corresponds to the target probability of DLTs to be used during and at the end of the trial, and the other, slot \code{Gstarderive}, will be used to derive the estimates of Gstar, the dose which gives the maximum gain value. In here, we will recommend 30\% posterior quantile of the samples to be estimates for the doses corresponding to the \code{DLEtargetDuringTrialtarget} and \code{DLEEndOfTrialtarget} doses and 50\% posterior quantile of the samples to be estimates for the doses corresponding to the Gstar, the dose which gives the maximum gain value.

In this package, the method \code{nextBest} can be applied to any of the classes defined under the parent \texttt{NextBest} class, to compute the next best dose during the study.
...
This is an example when no DLT and efficacy samples are involved. For cases where samples are involved, additional slots of \code{samples} for DLT samples and \code{Effsamples} have to be specified in the \code{nextBest} method. From the example above, the escalation rule (Class) is \texttt{GainNextBest}, the maximum allowable dose is specified in slot \code{doselimit}, the DLT model in slot \code{model}, the efficacy model in slot \code{Effmodel} and the data set in slot \code{data}. In addition, we can use the \verb|$| operator to look at specific content from the results. For example,
<<doseRecommendation>>=?
...
In particular, the 95\% credibility interval (CI) of the selected quantity obtained after each cohort of subjects treated will be used to decide if we have already obtained an 'accurate enough' estimate. The wider the CI, the less accurate of this CI. More specifically, the ratio of the upper to the lower limit of this CI is calculated. If this calculated ratio is less than or equal to a pre-specified value, the target ratio, the trial will stop. The smaller the value of the calculated ratio, the more accurate of the estimate. Therefore, the smaller the value we specify for the target ratio, the more precise the estimate we can obtain. To note, the number of subjects to recruit in a study will increase when more precise estimation are required.\\
...
The \texttt{StoppingMinPatients} class objects can be used to stop the trial if 20 patients have already been enrolled. For  \texttt{StoppingGstarCIRatio}, the value of the target ratio have to be specified in the \code{targetRatio} slot and slot \code{targetEndOfTrial} for the value of the target probability of DLTs to be used at the end of the trial is also required. There are more \texttt{Stopping} classes defined in this package. Please refer to the help pages for further details.\\

\textbf{Simulations}\\

In order to access the operating characteristics of the dose-escalation procedure, we can simulate a trial for large number of times and examine the results obtained from simulations. In our package, a design has to be built before running the simulations. The design consists of model(s), the escalation rules, starting data, a cohort size and a starting dose. In particular for the escalation rule, we have to specify the \texttt{Increments}, the maximum allowable increments between doses; \texttt{NextBest}, the rule to derive the next best dose; \texttt{CohortSize}, the number of subjects treated each time; and \texttt{Stopping}, the stopping rule to stop the dose-escalation procedure. All increments rules in this package are contained in the \texttt{Increments} class.
<<Increments>>=
myIncrements <- IncrementsRelative(intervals=c(25,300),
                                 increments=c(2,2))
@

The \texttt{IncrementsRelative} is one of the class object under \texttt{Increments} class. This is a class where the maximum relative increments will be calculated based on the current dose. In the example, we specify a maximum increase 200\% for doses up to 25mg and 200\% for dose above 25mg. In other words, this specification means a maximum of 3-fold increase for dose levels between 25 to 300 mg are defined.

We also need to define the cohort size for our study and a size of 3  will be used in our example. We can use \texttt{CohortSizeConst} class object if a fixed cohort size is used throughout the study.
<<cohortsize>>=
mySize <- CohortSizeConst(size=3)
@

Please also refer to the help pages for further details about other classes defined under \texttt{CohortSize}.

Then the design for the simulations can be defined. The class objects \texttt{TDDesign}, \texttt{TDsamplesDesign}, \texttt{DualResponsesDesign} and \texttt{DualResponsesSamplesDesign} can be used with models defined under \texttt{ModelPseudo} class. We will show an example on how we define a design when both DLTs and efficacy responses are incorporated into the procedure such that the \texttt{DualResponsesDesign} class object will be used. We want to start the simulations with an empty data set and start with the lowest dose 25 mg
...
design1 <- DualResponsesDesign(nextBest=GainNextBest,
                               ...
                               startingDose=25)
@
From the example, the dose escalation rule, \code{GainNextBest}; the DLT model, \code{DLTmodel}; the efficacy mode, \code{Effmodel}; the data set, \code{data2}, the stopping rule, \code{myStopping1}, the increment rule, \code{myIncrements}, the cohort size, \code{mySize} and the starting dose 25mg, have to be all specified in corresponding slots of the \texttt{DualResponsesDesign} class function. Then we have to specify the scenario for the simulation, that is the true DLT and efficacy curves that we will using.
...
@
Functions such as \code{curve} and \code{plot} can always be used to draw the real DLT, efficacy and gain curves.
...
From the example, we specify the true value of the precision (\code{trueNu}) of the efficacy response as 1/0.025 which is a true variance of value 0.025. The slot \code{arg} could contain additional arguments for the \code{trueDLE} or \code{trueEff} function and default as \code{NULL}. The number of simulations are specified in \code{nsim} and the random number generator seed in \code{seed}. The slot \code{parallel} can be used to enable running the simulations in parallel with all processors in the computer. The results \code{Sim1} will be put into one of the sub-class defined under the parent class \texttt{GeneralSimulations}. Using the function \code{class}, we will know that the simulated results is of \texttt{PseudoDualSimulations} class object. This class is a subclass of \texttt{PseudoSimulations} class object which is a subclass of \texttt{GeneralSimulations} class object. The descriptions of slots contains in the simulated results can be found at help pages of the corresponding class objects. In particular, the \code{data} slot contain a list of data produced of the simulated trial. For example, we can plot the course of the fourth simulated trial and find the final recommended dose of this trial by
<<plot-sim>>=
...
The plot for \code{Sim1} gives an overview of the final dose recommendations, trial trajectories and a box plot of the efficacy variance. A textual summary is given using the \code{summary} function given the \code{trueDLE} and \code{trueEff}. The summary of our simulations can also be shown graphically by applying \code{plot} to the summary. The plots of the summary gives an overview of the frequency (in percentage) for the total number of subject treated, the MTD estimates, proportion of DLTs observed and number of subjects treated above target across the simulated trials. The average estimated DLTs and efficacy curve are also given to compare to their corresponding real curves.
...
As described, the \pkg{crmPack} is constructed using S4 classes and methods. This allows users to extend our package easily by constructing their own codes by setting up new S4 classes and corresponding methods that can be used in junction with other existing classes and methods defined in the package.
...
In \citep{oquigley1990}, the continual reassessment method (CRM) is proposed. This is one of the most well-known procedures used for phase I dose-escalation trials. Assuming the risk of DLTs increases with doses, a one-parameter model is often assumed under the CRM to describe the relationship between the binary DLT responses and their corresponding dose levels.
In the current version of the \pkg{crmPack}, only the two parameter logistic regression model of class \texttt{LogisticIndepBeta} are defined under \texttt{ModelTox} class. An example will be given to demonstrate how the one-parameter model can be implemented in \pkg{crmPack}. Users can use similar techniques to set up their own S4 classes and methods to implement into the package. 

Suppose there are dose levels $d_{(1)} < ...< d_{(k)}$, the prior guess of corresponding probabilities of the occurrence of DLTs at dose $d_{(j)}$ is denoted by $\pi_j$ such that $\pi_1 < ...<\pi_k$. The one-parameter power model introduced in \citep{oquigley1990} will be used such that the probability of the occurrence of DLTs, $p(d_{(j)})$ at dose $j$ will be given as \begin{equation}
p(d_{(j)}) = \pi^{\theta}_j
\end{equation}
...
This class \texttt{OneParaExponential} is contained in the \texttt{ModelTox} class and inherits all methods and slots from \texttt{ModelTox} class. The slots can be defined by users within \code{representation} and the initial value of these slots can also be specified within \code{prototype}.
...
Methods inherited from \texttt{ModelTox} class such as \code{dose} and \code{prob} can also be used for \texttt{OneParaExponential} class as shown.
<<dose-prob>>=
