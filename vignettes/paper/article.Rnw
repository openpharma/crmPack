...
The prior of the model parameters are expressed in terms of pseudo data for \texttt{LogisticIndepBeta} class model. The slot \code{DLEdose} represented the two dose levels that we fixed, 25 and 300mg. The vector $c(3,3)$ specofied in the slot \code{DLEweights} means that assummed 3 subjects are treated at each of the two dose levels. The slot \code{binDLE} represented the number of DLTs 1.05 and 1.8 observed at the two corresponding dose levels out of the 3 subjects treated at each of the two dose levels. Since no real observations are available before the trial start, we will specify the \code{data} slot of this model with the empty data set \code{data1} of class \texttt{Data}. Note that, since at least two pseudo responses from two different dose levels are need to obtain the prior modal estimate of the intercept and the slope of the model, slots \code{binDLE}, \code{DLEweights} and \code{DLEdose} must be specified in form of a vector of at least length 2 and the order of their elements must be consistent to the other two vectors.


Furthermore, the structure of this model can be viewed using \code{str(DLTmodel)} and access to their slots using the \verb|@| operator. For example, we can obtain the prior modal estimate of $\phi1$ the intercept parameter of this model by
\code{DLTmodel@phi1}.

If more observations are available, a new data set can be specified. Then the \code{update} function can be used to update the current model and obtain updated posterior modal estimates using the \verb|@| operator. 
...
A study will be stopped when all dose levels under consideration are unsafe. Therefore, we will not allow subjects to administer to dose levels if the risk of DLTs us unacceptable. The safety constriants to be used are described in \cite{yeung2015}.
Two DLTS limits for DLTs risk (probability of the occurrenc of DLT) are fixed.: 1) $\gamma^{**}$ is the highest risk to be faced by subjects during the trial and 2) $\gamma^{*} \leq \gamma^{**}$ is the highest risk for subsequently treated subjects for future studies. The corresponding limiting doses are $d^*= {\rm TD}100\gamma^*$ and $d^{**}={\rm TD}100\gamma^{**}$, respectively. The slightly higher risk of DLE allowed during the study will allow
...
In general under our package, we can apply the method \code{nextBest} to any of the classes defined under the parent \texttt{NextBest} class, in order to compute the next best dose for our study.
...
This is an example when no DLT and efficacy samples are involved. For cases where samples are involved, additional slots of \code{samples} for DLT samples and \code{Effsamples} have to be specified in the \code{nextBest} method. From the example above, the escalation rule \texttt{GainNextBest}, the maximum dose in slot \code{doselimit}, the DLT model in slot \code{model}, the efficacy model in slot \code{Effmodel} and the data in slot \code{data} have to specified for the \code{nextBest} method. In addition, we can use the \verb|@| and \verb|$| operator to look at the structure or to obtain output value and plots. For example,
<<doseRecommendation>>=
...
The \texttt{StoppingMinPatients} class objects can be used to stop the dose escalation if a minimum overall number of patients of 20 have been enrolled. The \texttt{StoppingGstarCIRatio} is the class object that can be used to stop the trial when the ratio of the upper to the lower 95\% CI of the selected quantity (minimum of $d^*$ and $g^*$) is less than equal to 5, specified in the \code{targetRatio} slot. In addition, the slot \code{targetEndOfTrial} for the target probability of DLTs to be used at the end of the trial are also specified with the value 0.3. There are more \texttt{Stopping} classes defined in this package. Please refer to the examples presented in the vignettes or in the help pages for further details.\\
...
After establishing the real scenario, the trial can be simulated. The function \code{simulate} is used to simulate trial based a a specified scenario.
...
From the example, we specify the true precision (\code{trueNu}) of the efficacy response be 1/0.025 which also means using a variance of true value 0.025. The argument \code{arg} could contain additional arguments for the \code{trueDLE} or \code{trueEff} function and default as \code{NULL}. The number of simulations are specify with \code{nsim} and the random number generator seed with \code{seed}. The argument \code{parallel} can be used to enable running the simulations in parallel with all processors in the computer. The results \code{Sim1} will be put into one of the sub-class defined under the parent class \texttt{GeneralSimulations} within the package. Using the function \code{class}, we can see that the simulated results is a \texttt{PseudoDualSimulations} class object. This is a subclass of \texttt{PseudoSimulations} class object which is also a subclass of the \texttt{GeneralSimulations} class object. The descriptions of slots contains in the simulated results can be found at help pages of the corresponding class objects. In particular, the \code{data} slot contain a list of data produced of the simulated trial. For example, we can plot the course of the fourth simulated trial and find the final recommended dose of this trial by
<<plot-sim>>=
...
The plot for \code{Sim1} gives an overview of the final dose recommendations, trial trajectories and a box plot of the efficacy variance. A textual summary is given using the \code{summary} function given the \code{trueDLE} and \code{trueEff}. Finally, summary of our simulations can also be shown graphically by applying \code{plot} to the summary. The plots of the summary gives an overview of the frequency (in percentage) for the total number of subject treated, the MTD estimates, proportion of DLTs observed and number of subjects treated above target across the simulated trials. The average estimated DLTs and efficacy curve are also given to compare to their corresponding real curves.
...
As described, the \pkg{crmPack} is constructed using S4 classes and methods. This allows users to extend our package easily by constructing their own codes to set up new S4 classes and corresponding methods that can be used in junction with other existing classes and methods within the package.

One of the main framework of the S4 classes and methods defined in this package is \texttt{Model}. In this package, all dose-response models are contained in \texttt{AllModels} class and in either of the two subclasses of \texttt{AllModels}, \texttt{GeneralModel} or \texttt{ModelPseudo}. The models (classes) defined under \texttt{GeneralModel} involved MCMC sampling to obtain estimates for their model parameters where model parameters of models defined under \texttt{ModelPseudo} can be obtained without involving MCMC sampling. Two further subclasses, \texttt{ModelTox}, a class which contains all models for dose-toxicity relationship and \texttt{ModelEff}, a class which contains all model for dose-efficacy relationship are defined under \texttt{ModelPseudo}.


In \citep{oquigley1990}, the continual reassessment method (CRM) is proposed. This is one of the most well-known procedure used for phase I dose-escalation trials. Assuming the risk of DLTs increases with doses, a one-parameter model is often assumed under CRM to describe the relationship between the binary DLT responses and their corresponding dose levels.
...
This class \texttt{OneParaExponential} is defined as extending the \texttt{ModelTox}. In other words, class \texttt{OneParaExponential} inherits all methods and slots from \texttt{ModelTox} class. The slots can be defined by users using \code{representation} and the initial value of these slots can also be specified using \code{prototype}.
...
Users can also customized or set up other methods corresponds to this model class using the \code{setMethod} commands.
