...
- introduce basic approach for implementing N-CRM trial - use the same motivating example as Winnie

\subsection{Dose escalation with safety and efficacy}


In this section, the dose-escalation procedures where two, the safety and efficacy endpoints are incorporated into the study will be introduced. The safety endpoint is the binary DLTs (dose-limiting toxicity) responses and the efficacy endpoint is the continuous biomarker/efficacy response. In this package, either these two responses are modelled jointly, using a single model class and assuming correlation; or are modelled separately using two separate model classes and assuming no correlation. First, the procedure where the two endpoints are modelled separately will be described. 

\textbf{Two Endpoints modelled with two dose-response models}
...
The \texttt{ModelPseudo} class contains all model classes where their prior are specified in terms of pseudo data and standard maximum likelihood procedures from R can be used to obtain prior and posterior estimates. All models included in this package will have a parent class of either \texttt{GeneralModel} or \texttt{ModelPseudo}. In particular, all dose-responses models that can be used when the two endpoints are modelled separately are contained in \texttt{ModelPseudo} class.
...
\subsubsection{Two Endpoints are jointly modelled}
