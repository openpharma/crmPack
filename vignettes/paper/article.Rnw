...
Importantly, they avoid fixing only a few dose levels in advance.
...
\todo{Thomas: Explicit comparison against R-package bcrm - do we need more here?}
...
\label{sec:frame:model}
%
The core of model-based dose escalation designs is the model. Taking the
Bayesian inference approach, the model in \pkg{crmPack} consists of firstly the
...
reference dose $x^{*}$. The prior $p(\theta \given \xi)$ is typically specified
...
%
The other core elements of a dose escalation design concern the decision making
for the next dose $x_{N+1}$ to be tested in the next cohort. In the
...
levels, which can calculate from the current data $\mathcal{D}_{N}$ and current
dose $x_{N}$ the maximum possible next dose $t_{N+1}=\tau(\mathcal{D}_{N},
x_{N})$ for the next cohort. The second element is a rule $\nu$ indirectly
...
design and to convince oneself of its properties. In addition, the
\code{examine} method evaluates hypothetical trial outcomes and lists the
...
\todo{do we need a reference here?} For each patient, we observed a binary
safety (DLT) and a continuous efficacy outcome.
% The DLT response is equal to 1 if any abnormal symptom is observed and 0 otherwise.
...
novel agent are available, defining our dose grid $\mathcal{X} = \{25, 50,
\dotsc, 300\}$, with $x_{1} = 25$ mg being our starting dose. Our primary
objective in conducting the dose escalation study is to characterize the
dose-toxicity relationship and to estimate the MTD. We choose to use in this
case a logistic regression with likelihood~\eqref{eq:LogisticLogNormal} and
prior~\eqref{eq:bivariateNormal}.

\paragraph{Logistic regression model with minimally informative prior}
...
as shown in Section~\ref{sec:frame:model}. Here we assume only very limited
information is available and we use a \emph{minimally informative prior}
...
The resulting model with prior parameters $\mu = (\Sexpr{round(model@mean[1], 2)},
...
internally, setting of a seed is highly recommended.
...
<<init-Data>>=
data <- Data(x=c(25,25,25,50,50,50,100,100,100),
...
The argument \code{x} takes the doses $x_{1}=25, x_{2} = 50, x_{3} = 100$ (note
...
cohorts (here $N=3$). Patient IDs can be given optionally in the \code{ID}
argument. The data can then be visualized by simply applying the \code{plot}
function to the object:

<<plot-data,fig.width=4,fig.height=4,out.width='.3\\linewidth',fig.align='center'>>=
plot(data)
@
...
On the DLT rate scale the distribution can be visualized by supplying
the samples, model and data to the generic \code{plot} function, which generates
the posterior mean curve and 95\% equi-tailed credible intervals at each point
of the dose grid. Note that you can also produce a similar plot without any
data, which is giving the prior:
