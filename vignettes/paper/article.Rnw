...
Firstly the model parameters are updated with the \texttt{update} function and subsequently the next recommended dose found via \texttt{nextBest} specifying the rule to use the gain function (\texttt{GainNextBest}). 
