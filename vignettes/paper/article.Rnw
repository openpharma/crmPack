...
\proglang{SAS} \citep{SAS2003}, \code{glm} in \proglang{Stata} \citep{Stata2015} or \proglang{R} \citep{R2016}, 
...
framework based on the \proglang{S4} classes and methods system \citep{Chambers2008} and \pkg{JAGS} \citep{JAGS}
...
the likelihood and prior distributions in the underlying \code{Model} in \pkg{JAGS}.
...
code in the package - only minimal \proglang{S4} and \pkg{JAGS} knowledge is required.
\end{enumerate}
