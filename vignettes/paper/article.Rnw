...
%% \todo{Thomas: Explicit comparison against R-package bcrm - do we need more here?} -> leave it with that for now
...
\todo{rework this paragraph in the end - or consider dropping it completely}
...
\todo{do we need a reference here?} in order to illustrate the functionality.
...
\dotsc, 300\}$, with $x_{1} = 25$ mg being our starting dose. We choose to use in this
case a logistic regression with likelihood~\eqref{eq:LogisticLogNormal} and
prior~\eqref{eq:bivariateNormal}.

\todo{Giuseppe: use placebo in this Example - stick with the log(dose)}
...
\paragraph{Decision making for the next dose}
We can specify the function $\tau$ with maximum relative increments of 100\% for last doses below 100 mg, 50\% for last doses in the range from 100 mg to 200 mg, and 33\% for last doses equal or above 200 mg with the class \code{IncrementsRelative}:
<<rel-incrs>>=
...
In this case we will use a fixed cohort size of 3 patients throughout the study:
<<size-rules>>=
...
\todo{this is enough, we don't need joint model as well}
