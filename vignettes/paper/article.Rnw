...
\todo{Below max.time=1. Once the paper is finalized
...
data3 <- DataDual(x=c(25, 50, 50, 75, 100, 100, 200, 200), 
                  y=c(0, 0, 0, 0, 1, 1, 1, 1), 
...
<<Effmodel>>=
emptydata2 <- DataDual(doseGrid=seq(25, 300, 25))
## todo: ideally we want here 
...
<<nextBest>>=
(nextMaxDose <- maxDose(myIncrements, data3))
...
in the package. As for all designs, the underlying structure to extend to novel dual endpoint methods is hence provided. Simulation facilities for all approaches and relevant
