...
$c \geq 0$ is a constant. Usually the default value $c=0$ can be used, but in our case we choose $c=2$ to allow the placebo dose $x=\Sexpr{PL}$ which is close to 0. 
