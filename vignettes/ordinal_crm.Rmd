...
*  The `datablock` slot has been introduced to the `GeneralModel` class.  This is used to create a JAGS data block.  This is unnecessary for all classes other than `DataOrdinal` and so returns an empty string.  For `DataOrdinal`, it transforms the observed vector of participant-level toxicity grades into the matrix of flags indicating whether or not the participant experienced a toxicity of the given grade _or higher_.  There are minor consequential changes to `h_jags_write_model` function.
