...
Please test the newly written extensions with one CPU core, before utilizing the function `crmpack_extensions`. The submitted code can not be checked for validity and is executed directly at each core. In case of errors the returned error message may not help to identify the root problem. Debugging of mewly written code is much easier without using parallel computing.

## Worked out example

Lets assume we want want to use a logistic model with normal prior that is truncated for the slope parameter, so that the slope can have only positive values. This model representation is not part of the crmPack, but can easily be added. Please note that for this example the extra code is directly embedded into the `crmpack_extensions` function. As mentioned before, user should write and test their code first without embedding into the `crmpack_extensions` function to assure that no problem occur during general execution.
