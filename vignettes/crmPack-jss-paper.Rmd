...
\proglang{SAS} \citep{SAS2003}, \pkg{glm} in \proglang{Stata} \citep{Stata2015} or `R` \citep{R2016},
...
the  \pkg{dfcrm} package \citep{dfcrm2013} in `R` which additionally implements the
time-to-event CRM (TITE-CRM) \citep{cheung2000}. Several `R`-packages
...
In this paper we introduce the `R`-package `crmPack` \citep{crmPack} for
...
(shown as gray boxes) and bundled together in the overarching `Design` object.
...
%
In `crmPack` the \proglang{S4} class `GeneralData` encapsulates this
...
Bayesian approach to inference, the model in `crmPack` consists of firstly the
...
hyperparameters $\xi$.
%
In `crmPack` the virtual \proglang{S4} class `GeneralModel` encapsulates this
...
Section~\ref{subsec:extending} gives an example on extending the `crmPack`
functionality.
...
library("crmPack")
```
% As indicated in the startup message, try `crmPackHelp()` and
...
```{r min-inf, warning=FALSE, message=FALSE, results=hide}
...
The resulting \code{model} (which is an object of class `LogisticLogNormal`)
has prior parameters

$\mu = (\Sexpr{runif(1)}, \Sexpr{2})$
% $\mu = (\Sexpr{round(model@params@mean[1], 2)}, \Sexpr{round(model@params@mean[2], 2)})$ and
% $\Sigma=
% \left(\begin{smallmatrix}
%   \Sexpr{round(model@params@cov[1, 1], 2)} & \Sexpr{round(model@params@cov[1, 2], 2)} \\
%   \Sexpr{round(model@params@cov[2, 1], 2)} & \Sexpr{round(model@params@cov[2, 2], 2)}
...
observations $y$, the \proglang{S4} class `Data` can be used. Objects of this class
...
then provided to the `code` function, together with the \code{data} and the
\code{model} objects:
...
```
where \code{prob_target_drt} specifies the maximum estimated DLT rate tolerated
during the study and \code{prob_target_eot} the maximum estimated DLT rate tolerated
...
than~\Sexpr{myStopping4@target_ratio}.
...
\subsection[Extending crmPack functionality]{Extending `crmPack` functionality}
...
To implement the one-parameter model~\eqref{eq:oneparameter} in `crmPack`
we first need to define an appropriate \proglang{S4} class inheriting from the
...
\proglang{JAGS}. We choose to let the user supply just the skeleton probabilities
along with the intended dose grid to use. The likelihood with the power model
is specified in \code{datamodel} and uses the `Data` slots which are
specified in \code{datanames}. The prior is defined in \code{priormodel}.
Model parameters are passed to \proglang{JAGS} via \code{modelspecs}.
The \code{init} slot contains a function giving the starting values for the MCMC
sampler, and \code{sample} defines which parameter samples will be returned:
...
Finally, we will create the \code{dose} and \code{prob} methods. The \code{prob}
...
The \code{dose}, on the other hand, uses an inverse of (the skeleton) function
\begin{equation}
...
In order to make it usable we need to define the \code{nextBest} method for this
...
```{r newNextBest-method, results=hide}
setMethod(
  "nextBest",
  signature = signature(
