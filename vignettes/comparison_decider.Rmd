...
prob_samples_A_decider_formula <- plogis(
...
prob_samples_B_decider_formula <- plogis(
...
all_formula_probabilities <- c(
    prob_samples_A_decider_formula,
    prob_samples_B_decider_formula
)
downstream_check <- c(
    max_abs_probability_difference_A = max(abs(
        prob_samples_A_crmPack - prob_samples_A_decider_formula
    )),
    max_abs_probability_difference_B = max(abs(
        prob_samples_B_crmPack - prob_samples_B_decider_formula
    )),
    draws_exactly_at_0.33 = sum(all_formula_probabilities == 0.33),
    max_overdose_difference_due_to_boundary = max(abs(
        colMeans(prob_samples_B_decider_formula >= 0.33) -
            colMeans(prob_samples_B_decider_formula > 0.33)
    ))
)
...
small systematic sign. The remaining differences are consistent with
independent Monte Carlo error from Stan and JAGS. In particular, the most
visible small displacement in the plots is shared by the compound 1 parameters
in Arms A and B, as expected because those parameters use the same hierarchical
pool. Because an MCMC draw moves the entire dose-toxicity curve coherently, its
Monte Carlo error is also expected to have a coherent direction across doses.
The parameter-density plots above provide a more direct check: a meaningful
model mismatch would appear as non-overlapping or persistently shifted
parameter posteriors, rather than only as differences of a few thousandths in
correlated tail-area estimates.
