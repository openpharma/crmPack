...
probabilities. The longer run above brings the results much closer, but the
comparison should still be interpreted together with the different MCMC
parameterizations: `decider` uses a non-centered Stan parameterization, whereas
`crmPack` currently uses a centered JAGS parameterization.
...
likelihoods are equivalent. The probabilistic models therefore match, while
their centered and non-centered MCMC implementations can have substantially
