...
my_size <- maxSize(
  ...
  CohortSizeDLT(intervals = c(0, 1), cohort_size = c(1L, 3L))
)

my_stopping <- (StoppingMinCohorts(nCohorts = 3L) &
...
```{r}
vignetteMcmcOptions <- McmcOptions(burnin = 100L, step = 2L, samples = 1000L)
postSamples <- mcmc(
  data = firstFour,
  model = model,
