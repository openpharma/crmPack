...
>The `knit_print`methods provided by `crmPack` are not intended to be fully customisable or comprehensive.  We do believe, however, that they cover the vast majority of use-cases and are easily extended using the techniques described in @sec-override of this vignette.
...
[^2]: Because markdown header styles are defined only for six levels, the greatest value for `level`, including values generated by nested calls, is `6`.
...
You might feel this is better presented as a bulleted list.  You can achieve this as follows:
...
allClasses <- df %>% nrow()
knitPrintDefined <- df %>% filter(KnitPrint) %>% nrow()
knitPrintNotDefined <- allClasses - knitPrintDefined
virtualClasses <- df %>% filter(Constructor != "OK") %>% nrow()
stillToDo <- df %>% filter(Constructor == "OK", !KnitPrint) %>% nrow()
```

`crmPack` defines `r allClasses` classes.  Custom `knit_print` methods exist for  `r knitPrintDefined` of them.  Of the remaining `r knitPrintNotDefined` classes, `r virtualClasses` are virtual classes that will never be directly instantiated by end users.  That leaves `r stillToDo` classes for which `knit_print` methods may be useful.  These classes are listed below.
...
The majority of these classes relate to simulation of the operating characteristics of CRM trials.  Reporting of this information is likely to need customisation that is beyond the scope of a simple function[^3].  

[^3]: The `crmPack` team is considering the creation of markdown or Quarto templates that may assist in this area, but consider this to be a long-term ambition.
