...
$$\binom{\alpha}{\beta} \sim N \left( \left[\begin{array}{rr}`r sprintf(" %0.3f", min_inf_model$model@params@mean[1])` \\ `r sprintf(" %0.3f", min_inf_model$model@params@mean[2])` \end{array}\right], \left[\begin{array}{rr} `r sprintf(" %0.3f", min_inf_model$model@params@cov[1, 1])` & `r sprintf(" %0.3f", min_inf_model$model@params@cov[1, 2])` \\ `r sprintf(" %0.3f", min_inf_model$model@params@cov[2, 1])` & `r sprintf(" %0.3f", min_inf_model$model@params@cov[2, 2])`\end{array}\right]\right)$$
Note that `MinimalInformative` returns a `LogisticNormal` model, not a `LogisticLogNormal` model that we are using.  So we need to adjust its parameters accordingly.  For simplicity, adjust only the mean of &beta;, not the variance or correlation with &alpha;...

```{r}
mu <- min_inf_model$model@params@mean
mu[2] <- log(mu[2])
sigma <- min_inf_model$model@params@cov
revised_model <- LogisticLogNormal(mean = mu, cov = sigma, ref_dose = initial_model@ref_dose)
```
