...
To implement the one-parameter model~\eqref{eq:oneparameter} in \pkg{crmPack} we first need to define an appropriate \proglang{S4} class inheriting from the general model class \code{GeneralModel}:
<<classOnePara>>=
.OneParExp <-
    setClass(Class = "OneParExp", contains = "GeneralModel",
...
the likelihood and prior distributions in the underlying \code{GeneralModel} in \proglang{JAGS}.
...
OneParExp <- function(skeletonProbs, doseGrid, lambda)
{
  ...
  .OneParExp(
    ...
    dose = function(x, theta){ invSkeletonFun(x^(1 / theta)) },
