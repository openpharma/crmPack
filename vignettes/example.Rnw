...
Finally, you can install \texttt{crmPack} from the local zip file. The following
command assumes that the zip file is located in your current working directory:

<<install-crmPack, eval=FALSE>>=
install.packages("crmPack_0.0-9.zip", repos=NULL)
@
