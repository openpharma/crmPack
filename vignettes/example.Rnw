...
\date{\today\\Package version 0.0.22}
\maketitle
...
In the \texttt{Model-class} help page, you also find out that there are three
additional specific model classes that are subclasses of the \texttt{Model}
class, namely \texttt{LogisticNormal}, \texttt{LogisticKadane} and
\texttt{DualEndpoint}.
...
(here 50 mg) is not available, because it can happen that no patients were
...
that in 30\% of the simulated trials had 0, 5, or 10\% of the patient had DLTs,
respectively, while in 10\% of the simulated trials 18\% of the patients had
DLTs. The middle right panel shows the distribution across simulations of the
number of patients treated above the target toxicity window (here we used the
default from 20\% to 35\%). We see here that in all trials, none of the patients
were treated with too toxic doses. Finally, in the bottom panel we see a
comparison of the true dose-toxicity curve (black) with the estimated
dose-toxicity curves, averaged (continuous red line) across the trials and with
95\% credible interval across the trials. Here we see a good correspondence of
the truth and the estimation.
...
We see here e.g. that the estimated number of patients overall is 18, so 10 more
