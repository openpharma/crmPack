...
the internal MCMC computations. While JAGS is required for the default MCMC
runs, WinBUGS only needs to be installed currently if the dual-endpoint designs
need to be run (because they don't run with JAGS at the moment). To increase the
speed of the MCMC sampling, \texttt{BayesLogit} is currently used for the
...
the class specified by its first argument (here: \texttt{"LogisticLogNormal"}).
The \texttt{R}-package \texttt{crmPack} uses the S4 class system for
implementation of the CRM designs. We can query the class an object belongs to
with the \texttt{class} function:
...
options. By default, JAGS is used for obtaining the samples, and you could
specify \texttt{program="WinBUGS"} to use WinBUGS instead. This will dynamically
try to load the \texttt{R}-package \texttt{R2WinBUGS} to interface with WinBUGS.
Therefore you must have installed both WinBUGS and the R-package
