...
We note that here `refDose` should be chosen below or equal to the lowest dose, such that the 
term `log(dose / ref_dose)` is never negative and hence the probability of DLT
at any given dose will be higher for the combination than for the single agent.
...
LogisticLogNormalGrouped <- function(mean, cov, ref_dose = 1) {
  ...
  .LogisticLogNormalGrouped(
    ...
    datamodel = function() {
      for (i in 1:nObs) {
        logit(p[i]) <- (alpha0 + is_combo[i] * alpha0c) +
          (alpha1 + is_combo[i] * alpha1c) * log(x[i] / ref_dose)
        y[i] ~ dbern(p[i])
      }
    },
    ...
    sample = c("alpha0", "alpha0c", "alpha1", "alpha1c")
  )
}
```
...
setMethod(
  ...
  definition = function(x, model, samples, group) {
    ...
    ref_dose <- as.numeric(model@ref_dose)
    is_combo <- as.integer(group == "combo")
    exp((logit(x) - (alpha0 + is_combo * alpha0c)) / (alpha1 + is_combo * alpha1c)) * ref_dose
  }
)
```
...
mean(my_samples@data$alpha0c)
mean(my_samples@data$alpha1c)
...
one_sample <- Samples(
  data = list(alpha0 = -0.5, alpha0c = 0.1, alpha1 = 0.3, alpha1c = 0.5),
...
Here we add the staggering flag `staggered_cohorts`. When turned on, this means
that we first test one single agent cohort. Once that DLT data has been collected, we proceed
from the second cohort onwards with concurrent mono and combo cohorts.

We also add a flag `same_dose` to specify whether the lower dose, i.e. the combo dose, 
should be used as the next dose for both mono and combo. This might or might
not be desired in the given situation.
  ...
  def =
    function(object,
      ...
      runSim <- function(iterSim) {
        ...
        if (object@single@startingDose < object@combo@startingDose) {
          stop("combo starting dose cannot be higher than single starting dose")
        }
        this_single_dose <- if (object@same_dose) object@combo@startingDose else object@single@startingDose
        ...
        while (!(stop_single && stop_combo)) {
          if (!stop_single) {
            ...
            this_single_size <- size(
              object@single@cohort_size,
          ...
          if (!stop_single) {
            ...
            ## => what is the next best dose for single?
            ...
            stop_single_results <- crmPack:::h_unpack_stopit(stop_single)
          }
          ...
          if (!stop_combo) {
            ...
            if (object@same_dose) {
              if (this_single_dose < this_combo_dose) stop("combo dose cannot be higher than single dose")
              this_single_dose <- this_combo_dose
            }
          }
        }
        ...
        thisResult <- list(
          single = list(
            ...
            report_results = stop_single_results
          ),
          combo = list(
            data = this_combo_data,
            dose = this_combo_dose,
...
plot(my_sims$single@data[[1]])
plot(my_sims$combo@data[[1]])
```

So here we have allowed the single and combo doses to be different for each cohort.
And we can now also try out forcing the same dose for the single and combo cohorts:
...
plot(my_sims_same_dose$single)
plot(my_sims_same_dose$combo)

plot(my_sims_same_dose$single@data[[1]])
plot(my_sims_same_dose$combo@data[[1]])
```

This looks ok. Note that when we have staggered the first cohort as here and limit
the sample size for single and combo at the same maximum then we might stop single
earlier than combo. 
