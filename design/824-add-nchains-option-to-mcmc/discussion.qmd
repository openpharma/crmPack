...
Clearly, what values can be considered extreme depend on the prior distribution of the model. These
...
By modifying the
function`in a`Model`'s`init`slot to take a parameter that defines the number of chains required, multiple`inits\`
can be easily supported without breaking existing code:
