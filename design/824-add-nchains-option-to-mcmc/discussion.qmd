...
> The `options` slot of the `Samples` class will need modification.

`h_jags_extract_samples` appears to assume that there will be only a single chain:

```{r}
#| label: h_jags_extract_samples-1
#| eval: FALSE
...
So modify it. [This is a temporary fix.]

```{r}
#| label: h_jags_extract_samples-2
#| eval: TRUE
...
There's a problem with `tidy`: it doesn't recognise multiple chains.  This might be because of the hacky fix to `h_jags_extract_samples`.  But the observation count is correct.
