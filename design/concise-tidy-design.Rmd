...
h_slot_to_tibble <- function(obj, slot_name, col = NULL, attributes = FALSE) {
...
```{r, error = TRUE}
x <- CohortSizeConst(size = 3L) |> tidy()
x
class(x)
```
...
x <- CohortSizeParts(cohort_sizes = c(1L, 3L)) |> tidy()
x
class(x)
```
...
> The `cov` and `prec` slots of the `ModelParamsLogNormal` class both have an attribute set.  Unfortunately, it's called `dim` in both cases.  As might be expected, the value of `dim` is the same for both slots.  As `dim` is uninteresting, this is easily dealt with: by ignoring it.
