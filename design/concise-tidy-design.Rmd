...
[Issue 652](https://github.com/Roche/crmPack/issues/652)has introduced a common super class for all other `crmPack` classes.  This can be leveraged to provide a single `tidy()` method that will tidy the majority of class-to-tibble conversions in `crmPack`.  A helper function to manage interval conversions (as discussed under Exceptions above) would alllow almost every exception to the general rule to be handled in a simple, two line sub class-specific method.  
...
The elements of the `data` slot in a `Simulations` object have a simple set of attributes:
```{r}
options <- McmcOptions(
  burnin = 100,
  step = 1,
...
h_handle_attributes <- function(x, .ignore = c("names", "class", "description", "row.names")) {
  a <- attributes(x)
  valid_names <- names(a)[which(!(names(a) %in% .ignore))]
  lapply(
    valid_names,
    function(n) {
      z <- attr(x, n)
...
h_handle_attributes(y)

attr(y, "dummy") <- c(upper = 1, lower = 0)
h_handle_attributes(y)

attr(y, "dummyList") <- list(upper = 1, lower = 0)
h_handle_attributes(y)
```
