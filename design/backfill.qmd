...
- For specific stopping rules we can add a flag whether to include backfill patients or not.
  - For the beginning we can just implement this for the `StoppingPatientsNearDose` rule.
...
- We also need here an additional `truthResponse` argument which gives the probability of response for each dose because we need to generate responses, because the opening of backfill cohorts (might) depend on it.
...
design <- Design(
    ...
    backfill = crmPack:::Backfill(
        cohort_size = CohortSizeConst(3),
        recruitment = crmPack:::RecruitmentRatio(ratio = 1),
        opening = crmPack:::OpeningMinDose(min_dose = 1) &
            crmPack:::OpeningMinCohorts(min_cohorts = 3),
        total_size = 12,
        priority = "highest"
    )
)

myTruth <- probFunction(design@model, alpha0 = 3, alpha1 = 3)
...
# We can check some simulated trials, e.g.:
result@data[[1]]
```

Here we can see the backfilled patients.
...
- Also the timing of the backfilling can be discussed: 
  ...
  - See [here](https://github.com/openpharma/crmPack/blob/danielinteractive/prototype669/R/Design-methods.R) for the `simulate` method prototype.
  - Answer: That is fine.
