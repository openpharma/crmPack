...
PowerNormal <- function(skel_probs, dose_grid, mean, variance) {
    ...
    .PowerNormal(
      ...
      modelspecs = function() {
        list(skel_probs = skel_probs,
             mean = mean, variance = variance) },
      init = function() {
        list(param1 = 1) }, sample = "param1")
}

mcmc_options <- crmPack::McmcOptions(burnin = 50000, step = 2,
                                     samples = 1000000)


## One-paramter model
## (A) Posterior summaries (original skeleton)
empty_data <- crmPack::Data(dose_grid = c(1, 2.5, 5, 10, 15, 20, 25, 30,
                                        40, 50, 75, 100, 150, 200, 250))

data_obs <- crmPack::Data(x = c(rep(1, 3), rep(2.5, 4), rep(5, 5),
                                rep(10, 4), rep(25, 2)),
                          y = c(rep(0, 3), rep(0, 4), rep(0, 5),
                                rep(0, 4), rep(1, 2)),
                          cohort = c(rep(1, 3), rep(2, 4), rep(3, 5),
                                     rep(4, 4), rep(7, 2)),
                          doseGrid = c(1, 2.5, 5, 10, 15, 20, 25, 30,
                                       40, 50, 75, 100, 150, 200, 250),
                          ID = 1:18)

model_power_A <- PowerNormal(skel_probs = c(0.01, 0.015, 0.020, 0.025, 0.03,
                                              0.04, 0.05, 0.10, 0.17, 0.30,
                                              0.45, 0.70, 0.80, 0.90, 0.95),
                             dose_grid = c(1, 2.5, 5, 10, 15, 20, 25, 30,
                                           40, 50, 75, 100, 150, 200, 250),
                             mean = 0,
                             variance = 1.34^2)
...
ncrm_loss <- crmPack::NextBestNCRMLoss(target = c(0.2, 0.35),
                               overdose = c(0.35, 0.6),
                               unacceptable = c(0.6, 1),
                               max_overdose_prob = 0.25,
                               losses = c(1, 0, 1, 2))

increments_no <- crmPack::IncrementsRelative(intervals = 250,
                                    increments = 2)

post_samples_A <- crmPack::mcmc(data_obs, model_power_A, mcmc_options)

dose_rec_loss_A <- crmPack::nextBest(ncrm_loss,
                                    doselimit = crmPack::maxDose(increments_no,
                                                                 data_obs),
...
## (A) Actual table I
pat_info <- rbind("No. of patients" = c(3, 4, 5, 4, "-", "-", 2, "-", "-", "-"),
                  "No. of DLTs" = c(0, 0, 0, 0, "-", "-", 2, "-", "-", "-"))
colnames(pat_info) <- c(1, 2.5, 5, 10, 15, 20, 25, 30, 40, 50)

tab1A_act <- rbind("Skeleton (CRM)" = c(0.01, 0.015, 0.020, 0.025, 0.03,
                                        0.04, 0.05, 0.10, 0.17, 0.30,
                                        0.45, 0.70, 0.80, 0.90, 0.95)[1:10],
                         t(dose_rec_loss_A$probs[, c(6:7)])[, 1:10])

## (B) Actual table I
model_power_B <- PowerNormal(skel_probs = c(0.063, 0.125, 0.188, 0.250, 0.313,
...
tab1B_act <- rbind("Skeleton (CRM)" = c(0.01, 0.015, 0.020, 0.025, 0.03,
                                        0.04, 0.05, 0.10, 0.17, 0.30,
                                        0.45, 0.70, 0.80, 0.90, 0.95)[1:10],
                         t(dose_rec_loss_B$probs[, c(6:7)])[, 1:10])

## (A)+ (B) Actual table I
tab1_act <- list("Posterior summaries (original skeleton)" = tab1A_act,
                "Posterior summaries (equidistant skeleton)" = tab1B_act)


## Expected table I (Neuenschwander et al.)
tab1_exp <- list()
tab1_names <- list(c("Skeleton (CRM)", "Mean", "Std. dev."),
...
tab1_exp$"Posterior summaries (original skeleton)" <- apply(as.matrix(tab1[4:6, -1]), 2, as.numeric)
tab1_exp$"Posterior summaries (equidistant skeleton)" <- apply(as.matrix(tab1[8:10, -1]), 2, as.numeric)
colnames(tab1_exp[[1]]) <- colnames(tab1_exp[[2]]) <- tab1_names[[2]]
