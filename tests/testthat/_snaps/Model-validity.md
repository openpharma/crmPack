...
# v_model_logistic_normal_fixed_mix returns message for non-valid ModelParamsNormal comp
    ...
    Output
      [1] "components must be a list with valid ModelParamsNormal S4 class objects, mean must have length 2 and no missing values are allowed, cov must be 2x2 matrix without any missing values"
