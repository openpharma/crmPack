...
test_that("prob-numeric-LogisticLogNormalOrdinal fails gracefully with bad input", {
  ...
  suppressWarnings({ samples <- mcmc(ordinal_data, model, opts) })

  expect_error(prob(-3, model, samples, 1), "Element 1 is not >= 0.")
  expect_error(prob(1, model, samples, 1), "Assertion on 'grade' failed: Must be of type 'integer', not 'double'.")
  expect_error(prob(1, model, samples, -1L), "Assertion on 'grade' failed: Element 1 is not >= 0.")
  expect_error(prob(1, model, samples, grade = 1L, cumulative = "bad"), "Assertion on 'cumulative' failed: Must be of type 'logical flag', not 'character'.")
  expect_error(prob(1, model, samples, grade = 1L, cumulative = c(TRUE, FALSE)), "Assertion on 'cumulative' failed: Must have length 1.")
})
